...
def _run_registration(
    ...
    try:
        ...
        if not token or not token.get("access_token"):
            ...
            token_err_desc = None
            try:
                token_err = sso_import.get_last_sso_token_error()
            ...
            raise RuntimeError(
                ...
                "rate_limited/slow_down under concurrent registration, or "
                "new-account invalid_grant / Access denied). "
                f"adapter_build={ADAPTER_BUILD}; sso_set={bool(sso)}; "
                f"token_error={token_err}; "
