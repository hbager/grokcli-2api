...
def load_credentials(
    ...
    try:
        from grok2api.upstream.oidc_auth import ensure_fresh_entry, parse_expires_at as _parse_exp
    ...
    if creds.expired and not creds.refresh_token:
        ...
        try:
            from grok2api.upstream.oidc_auth import delete_account_for_refresh_failure
    ...
    if creds.expired and creds.refresh_token:
        try:
            from grok2api.upstream.oidc_auth import RefreshRevokedError, refresh_and_persist
...
def upstream_headers(token: str, model: str, conv_id: str | None = None) -> dict[str, str]:
    """Headers required by cli-chat-proxy (mirror Grok CLI / CPA)."""
    headers = {
    ...
    if cid:
        # CPA sets x-grok-conv-id for Grok prompt-cache locality.
        headers["x-grok-conv-id"] = cid
    return headers
