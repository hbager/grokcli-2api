...
import grok2api.protocol.anthropic_compat as anth
import grok2api.admin.apikeys as apikeys
import grok2api.pool.conversation_affinity as conversation_affinity
import grok2api.pool.model_health as model_health
import grok2api.protocol.openai_responses as oai_resp
...
def _on_startup() -> None:
    ...
    try:
        from grok2api.admin.settings_store import apply_runtime_settings_to_modules
    ...
    try:
        from grok2api.upstream.models import ensure_models_catalog_seeded
    ...
    try:
        from grok2api.store.leader import should_start_maintainers, status as leader_status
...
async def metrics():
    ...
    try:
        from grok2api.store.metrics import prometheus_text
...
def _reload_enabled() -> bool:
    ...
    try:
        from grok2api.config import RELOAD
...
def _reload_kwargs() -> dict:
    ...
    root = _config.APP_ROOT
    try:
        from grok2api.config import RELOAD_DIRS, RELOAD_EXCLUDES, RELOAD_INCLUDES
    except Exception:
...
def main() -> None:
    ...
    from grok2api.config import WORKERS
    ...
    try:
        from grok2api.store import require_high_concurrency_stores
