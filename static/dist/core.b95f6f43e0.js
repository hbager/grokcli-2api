...
const PAGE_META = {
  ...
  accounts: { title: "Refresh Token", sub: "OAuth / Build 账号、Token 续期、额度与模型测试" },
...
function accountSurface(pathname) {
  return String(pathname || location.pathname || "").replace(/\/$/, "").endsWith("/accounts-sso") ? "sso" : "oauth";
}

function ensureAccountSurfaceNav() {
  const oauth = document.querySelector('.g2a-menu-item[data-page="accounts"]');
  if (!oauth) return;
  oauth.href = "/admin/accounts";
  oauth.dataset.accountSurface = "oauth";
  const oauthLabel = oauth.querySelector("span:last-child");
  if (oauthLabel) oauthLabel.textContent = "Refresh Token";
  let sso = document.querySelector('.g2a-menu-item[data-account-surface="sso"]');
  if (!sso) {
    sso = oauth.cloneNode(true);
    oauth.insertAdjacentElement("afterend", sso);
  }
  sso.href = "/admin/accounts-sso";
  sso.dataset.accountSurface = "sso";
  sso.classList.remove("is-active", "active");
  const ssoLabel = sso.querySelector("span:last-child");
  if (ssoLabel) ssoLabel.textContent = "SSO";
}

let renderedAccountSurface = "";

function applyAccountSurfaceUI() {
  if ((document.body.dataset.page || pageFromPath(location.pathname)) !== "accounts") return;
  const surface = accountSurface();
  if (renderedAccountSurface && renderedAccountSurface !== surface) {
    accountsList = [];
    accountsTotal = 0;
    accountsTotalPages = 1;
    selectedAccountIds.clear();
  }
  renderedAccountSurface = surface;
  if (surface === "sso") {
    if (!["", "live", "disabled"].includes(accountsStatusFilter)) accountsStatusFilter = "";
    accountsSsoFilter = "";
  }
  document.querySelectorAll("[data-surface-only]").forEach((el) => {
    el.hidden = el.dataset.surfaceOnly !== surface;
  });
  const title = $("accounts-card-title");
  const desc = $("accounts-card-desc");
  if (title) title.textContent = surface === "sso" ? "SSO 账号" : "Refresh Token 账号";
  if (desc) desc.textContent = surface === "sso" ? "Console / Web SSO 测活、启停与批量运维。" : "OAuth / Build 续期、额度、模型探测与批量运维。";
  const cols = document.querySelectorAll("#accounts-table-head th");
  if (cols.length >= 9) {
    cols[2].textContent = surface === "sso" ? "SSO" : "Token";
    cols[3].textContent = surface === "sso" ? "凭证" : "类型";
    cols[5].textContent = surface === "sso" ? "请求" : "用量";
    cols[6].textContent = surface === "sso" ? "SSO 状态" : "额度使用";
    cols[8].textContent = surface === "sso" ? "Cookie" : "过期";
  }
  if (surface === "sso") stopQuotaLiveRefresh();
}
let _softNavToken = 0;
...
function pageFromPath(pathname) {
  ...
  if (p.endsWith("/accounts-sso")) return "accounts";
...
function setActiveMenu(page) {
  const activeHref = page === "accounts" && accountSurface() === "sso"
    ? "/admin/accounts-sso"
    : (PAGE_HREF[page] || "/admin");
...
function applyPageMeta(page) {
  const surface = accountSurface();
  const meta = page === "accounts" && surface === "sso"
    ? { title: "SSO", sub: "Console / Web 账号、SSO 测活与独立轮询状态" }
    : (PAGE_META[page] || PAGE_META.overview);
...
async function softNavigate(name, opts) {
  ...
  const href = opts.href || (page === "accounts" && accountSurface() === "sso"
    ? "/admin/accounts-sso"
    : (PAGE_HREF[page] || "/admin"));
  ...
  try {
    ...
    try { ensureAccountSurfaceNav(); } catch (_) {}
    try { buildMobileNav(); } catch (_) {}
    applyPageMeta(page);
    try { applyAccountSurfaceUI(); } catch (_) {}
    ...
    try {
      ...
      if (page === "accounts" && accountSurface() === "oauth" && typeof startQuotaLiveRefresh === "function") startQuotaLiveRefresh({ immediate: false });
    } catch (_) {}
    }
    if (page === "settings") {
...
function rebindPageControls() {
  ...
  if ($("btn-start-reg")) on("btn-start-reg", "onclick", async () => {
    try {
      ...
      // The start endpoint also persists registration config. Reset write-only
      // inputs and clear intents only after it accepted the request.
      resetRegSecretControls();
      saveRegTrack();
      ...
      try {
        ...
        if (btn.dataset.act === "toggle-acc") {
          ...
          try {
            ...
            await api("/accounts/" + encodeURIComponent(id) + "/enabled", { method: "PATCH", body: JSON.stringify({ enabled: en, surface: accountSurface() }) });
            toast(en ? "已启用" : "已禁用");
...
function buildMobileNav() {
  ...
  const activeHref = accountSurface() === "sso" && active === "accounts" ? "/admin/accounts-sso" : (PAGE_HREF[active] || "/admin");
  const items = [
    ...Object.keys(PAGE_META).map((key) => ({ key, href: PAGE_HREF[key], title: PAGE_META[key].title })),
    { key: "accounts-sso", href: "/admin/accounts-sso", title: "SSO" },
  ];
  const order = ["overview", "keys", "accounts", "accounts-sso", "usage", "logs", "models", "settings", "guide"];
  const byKey = Object.fromEntries(items.map((item) => [item.key, item]));
  host.innerHTML = order.map((key) => byKey[key]).filter(Boolean).map((item) => {
    let href = item.href || "";
    // Never emit empty / "undefined" / relative garbage — that was the mobile 404.
    if (!href || href === "undefined" || href === "null") {
      href = item.key === "overview" ? "/admin" : ("/admin/" + item.key);
    }
    if (href.charAt(0) !== "/") href = "/" + href;
    const on = href.replace(/\/$/, "") === activeHref ? "active is-active" : "";
    return `<a class="${on}" href="${href}" data-page="${item.key === "accounts-sso" ? "accounts" : item.key}">${item.title}</a>`;
  }).join("");
}
...
async function refreshOverviewStatus({ force = true, render = true } = {}) {
  // Refresh authenticated dashboard state so overview actions update immediately.
  try {
    if (force) _statusFetchedAt = 0;
    const dash = await api("/dashboard");
    dashCache = dash || dashCache;
...
async function loadDashboard(prefetchedDashboard = null) {
  const page = document.body.dataset.page || pageFromPath(location.pathname) || "overview";
  // Refresh public bootstrap status for setup/version. Overview state comes from /dashboard.
  try {
    const now = Date.now();
  ...
  if (page === "overview") {
    // Paint cached dashboard state immediately, then replace it from the protected endpoint.
    ...
    try {
      const dash = prefetchedDashboard || await api("/dashboard");
      dashCache = dash;
      if (window.G2A && G2A.state) G2A.state.dashboard = dashCache;
      try { renderStats(); } catch (e) { console.error(e); }
      try { renderMaintainer(); } catch (e) { console.error(e); }
      ...
      if (e && e.status === 401 && !e.soft) throw e;
      // Keep last dashCache if any; stats already rendered from cached state.
    }
  } else if (page === "keys") {
  ...
  try {
    ...
    const d = dashCache || {};
    const ver = st.version || d.version || "";
    if ($("app-version") && ver) {
    ...
    if (pill) {
      const mode = d.account_mode || "";
      const live = (d.accounts && d.accounts.active_count) ?? (d.pool && d.pool.live);
      const email = d.credentials_email || "";
      pill.className = "g2a-tag" + (d.credentials_ok ? " ok" : "");
...
function renderStats() {
  const s = statusCache || {};
  const d = dashCache || {};
  // Dashboard is the authenticated source for pool counters and admin state.
  const pool = Object.assign({}, s.pool || {}, d.pool || {});
  const credOk = !(d.credentials && d.credentials.error) && (d.credentials_ok || (pool.live > 0));
  const pill = $("status-pill");
  if (credOk) {
    pill.className = "g2a-tag ok";
    pill.textContent = "● 已登录 " + (d.credentials_email || "") + " · " + (d.account_mode || "");
  } else {
...
function poolCountForStatus(key) {
  if (accountSurface() === "sso") return null;
  const s = statusCache || {};
  const d = dashCache || {};
  // Dashboard is the authenticated source for pool counters.
  const pool = Object.assign({}, s.pool || {}, d.pool || {});
...
function renderAccountStatusChips() {
  ...
  const filters = accountSurface() === "sso"
    ? ACCOUNT_STATUS_FILTERS.filter((f) => !f.key || f.key === "live" || f.key === "disabled")
...
async function selectAllFilteredAccounts() {
  ...
  try {
    const data = await api(
      ...
      `&q=${encodeURIComponent(q)}&sort=${encodeURIComponent(sort)}${ssoQs}${statusQs}&surface=${encodeURIComponent(accountSurface())}`
    );
...
function accountRowPresentation(a, p, liveQ) {
  const sso = accountSurface() === "sso";
  ...
  if (sso) {
    return {
      state: a.has_sso ? '<span class="g2a-tag ok">有效</span>' : '<span class="g2a-tag bad">缺失</span>',
      type: '<span class="g2a-tag ok">SSO Cookie</span>',
      usage: "—",
      quota: '<span class="g2a-muted">独立于 OAuth</span>',
      credential: a.has_sso ? '<span class="g2a-tag ok">已保存 Cookie</span>' : '<span class="g2a-tag bad">无 Cookie</span>',
      actions: `<button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="probe-one" data-id="${esc(a.id)}">SSO 测活</button>
        <button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="toggle-acc" data-id="${esc(a.id)}" data-on="${enabled ? 0 : 1}">${enabled ? "禁用 SSO" : "启用 SSO"}</button>`,
    };
  }
  return {
    state: a.has_access_token ? '<span class="g2a-tag ok">有效</span>' : '<span class="g2a-tag bad">缺失</span>',
    type: fmtAccountTypeCell({ ...p, id: a.id, account_type: a.account_type || p.account_type, plan: a.plan || p.plan, plan_label: a.plan_label || p.plan_label, last_quota: p.last_quota }, liveQ),
    usage: `${p.success_count || 0}√ / ${p.fail_count || 0}× · 共 ${p.request_count || 0}`,
    quota: fmtQuotaCell({ ...p, id: a.id }, liveQ),
    credential: `${fmtExpiry(a.expires_at)}<div style="margin-top:6px">${refreshPill}</div>`,
    actions: `<button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="renew-one" data-id="${esc(a.id)}" ${a.has_refresh_token ? "" : "disabled title=\"无 refresh_token，无法续期\""}>续期</button>
        <button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="probe-one" data-id="${esc(a.id)}" ${a.has_access_token ? "" : "disabled title=\"无 OAuth bearer，无法测活\""}>Build 测活</button>
        <button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="quota-one" data-id="${esc(a.id)}" ${a.has_access_token ? "" : "disabled title=\"无 OAuth bearer，无法查询额度\""}>额度</button>
        <button class="g2a-btn g2a-btn-default g2a-btn-sm" data-act="toggle-acc" data-id="${esc(a.id)}" data-on="${enabled ? 0 : 1}" ${enabled || a.has_access_token ? "" : "disabled title=\"无 OAuth bearer，无法启用 OAuth\""}>${enabled ? "禁用 OAuth" : "启用 OAuth"}</button>`,
  };
}
...
function renderAccountsPage() {
  ...
  if (accountsLoading && !pageItems.length) {
    ...
    tbody.innerHTML = pageItems.map((a) => {
      ...
      const view = accountRowPresentation(a, p, liveQ);
    ...
    <tr data-acc-id="${esc(a.id)}">
      ...
      <td>${view.state}</td>
      <td style="min-width:88px">${view.type}</td>
      <td>${poolLabel}</td>
      <td class="g2a-muted" style="font-size:0.8rem">${view.usage}</td>
      <td style="font-size:0.82rem;min-width:150px">${view.quota}</td>
      <td style="font-size:0.78rem;min-width:160px">${probeCell}</td>
      <td style="font-size:0.8rem;min-width:150px">${view.credential}</td>
      <td class="g2a-actions">${view.actions}</td>
    </tr>`;
...
async function hydrateQuotaCacheFromDB() {
  if (accountSurface() !== "oauth") return;
...
function resolveAccountsListQuery() {
  ...
  const statusQs = accountsStatusFilter
    ? `&status=${encodeURIComponent(accountsStatusFilter)}`
    : "";
  const surfaceQs = `&surface=${encodeURIComponent(accountSurface())}`;
  const path =
    `/accounts?page=${encodeURIComponent(page)}&page_size=${encodeURIComponent(pageSize)}` +
    `&q=${encodeURIComponent(q)}&sort=${encodeURIComponent(sort)}${ssoQs}${statusQs}${surfaceQs}`;
...
async function loadAccountsPage({ reset = false, silent = false } = {}) {
  try { if (accountSurface() === "oauth") ensureQuotaCacheHydrated(); } catch (_) {}
  ...
  try {
    ...
    try {
      ...
      window.__g2aQuotaMissingT = setTimeout(() => {
        if ((document.body && document.body.dataset.page) !== "accounts" || accountSurface() !== "oauth") return;
    ...
    if (data.pool && typeof data.pool === "object" && Object.keys(data.pool).length) {
      ...
      // List response may omit pool on older builds — refresh protected dashboard counters.
...
async function deleteSelectedAccounts() {
  ...
  try {
    ...
    dashCache = await api("/dashboard");
    if (window.G2A && G2A.state) G2A.state.dashboard = dashCache;
    await loadDashboard(dashCache);
  } catch (e) {
...
function startQuotaLiveRefresh({ immediate = false } = {}) {
  const page = (document.body && document.body.dataset.page) || "";
  if (page !== "accounts" || accountSurface() !== "oauth") {
...
async function runAccountProbe(accountId, model) {
  ...
  try {
    const body = { auto_disable: true, surface: accountSurface() };
    ...
    const pool = r.pool || {};
    const recovered = accountSurface() === "sso"
      ? false
      : !!(res.recovered || (res.auto_action && res.auto_action.recovered))
    ...
    const cooling = accountSurface() === "sso" ? false : !!(
      res.kicked_cooldown
      || pool.in_cooldown
      || r.in_cooldown
      || pool.pool_status === "cooldown"
    ...
    const lines = [
      ...
      ok && recovered ? (accountSurface() === "sso" ? "状态：SSO 已恢复" : "状态：冷却中 → 正常（已写库）") : null,
    ...
    toast(
      ok
        ? (recovered ? (accountSurface() === "sso" ? "SSO 测活成功，已恢复" : "测活成功，已恢复为正常") : "账号模型探测成功")
        : (cooling ? "测活失败，已进入冷却中" : (res.error || r.error || "探测失败")),
      ok
    );
    const poolPatch = accountSurface() === "sso"
      ? {
          ...(res.auto_disabled ? { enabled: false, pool_status: "disabled" } : {}),
          last_probe: res,
          last_probe_status: ok ? "ok" : "fail",
...
async function probeAccounts(ids, { confirmMany = true } = {}) {
  ...
  try {
    const r = await api("/accounts/probe-batch", {
      method: "POST",
      body: JSON.stringify({ ids: list, auto_disable: true, surface: accountSurface() }),
    });
    ...
    results.forEach((item) => {
      ...
      const poolPatch = accountSurface() === "sso"
        ...
        : poolPatchFromProbeResponse(item);
      const topPatch = {
...
// Dashboard chips and list rows are independent: dashboard failure must not block the list.
async function refreshAccountsListUI({ toastOk = "", force = true } = {}) {
  ...
  try {
    ...
    if (e && e.status === 401) throw e;
    console.warn("refreshAccountsListUI /dashboard", e && (e.message || e));
  }
...
function startAutoUiRefresh() {
  ...
  uiRefreshTimer = setInterval(async () => {
    try {
      ...
      try {
        const now = Date.now();
        // Protected dashboard at most every 15s (passive overview refresh).
        if (!dashCache || (now - (_statusFetchedAt || 0)) > 15000) {
          dashCache = await api("/dashboard");
          _statusFetchedAt = Date.now();
        ...
        try { renderModelHealthInfo(); } catch (_) {}
        try { renderStoreConn("overview-conn"); } catch (_) {}
        const tm = (dashCache && dashCache.token_maintainer) || {};
...
// Only retain whether a provider secret exists. Secret values stay in the
// password inputs until submitted and are never copied into application state.
let regMailKeySet = { moemail: false, yyds: false, gptmail: false, cfmail: false, tempmail: false, cloudmail: false };
let regMailClearProvider = null;
...
function currentRegMailProvider() {
  ...
  if (mail === "cloudmail" || mail === "skymail" || mail === "cmail") return "cloudmail";
  return "moemail";
}
...
/** Pull every provider's non-secret dedicated inputs into memory. */
...
/** Write non-secret memory and secret-set status to dedicated DOM inputs. */
function paintRegMailFieldsToInput() {
  for (const mail of REG_MAIL_PROVIDERS) {
    const ids = REG_MAIL_INPUT_IDS[mail] || {};
    if (ids.key && $(ids.key)) {
      $(ids.key).placeholder = regMailKeySet[mail]
        ? "已保存，留空保留"
        : "未设置；输入新值后保存";
    }
    if (ids.domain && $(ids.domain)) {
...
function regMailProviderMeta(mail) {
  ...
  const table = {
    ...
    cloudmail: { title: "CloudMail / SkyMail", help: "Token + Base URL · doc.skymail.ink · /api/public/addUser + emailList", temp24h: false },
  };
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (t.id === "reg-mail-provider") {
        try {
          if (regMailClearProvider && currentRegMailProvider() !== regMailClearProvider) {
            if ($("reg-api-key-clear")) $("reg-api-key-clear").checked = false;
            regMailClearProvider = null;
          }
      ...
      if (t.id === "reg-api-key-clear") {
        if (t.checked) regMailClearProvider = currentRegMailProvider();
        else regMailClearProvider = null;
        return;
      }
...
function readRegConfig() {
  ...
  // Capture only non-secret provider fields. Blank secret inputs preserve the
  // server value unless the corresponding explicit clear control is checked.
  stashRegMailFieldsFromInput();
  regMailProviderPrev = mailProvider;
  const activeIds = REG_MAIL_INPUT_IDS[mailProvider] || {};
  const activeKey = activeIds.key && $(activeIds.key)
    ? _cleanSecretDisplay($(activeIds.key).value.trim())
    : "";
  ...
  // The proxy list may itself contain embedded credentials, so it is treated as
  // secret and is never copied into local cache by registrationConfigForLocalCache.
  if ($("reg-proxy")) cfg.proxy = $("reg-proxy").value.trim();
  if ($("reg-proxy-username")) cfg.proxy_username = $("reg-proxy-username").value.trim();
  if (activeKey && !($("reg-api-key-clear") && $("reg-api-key-clear").checked)) {
    cfg.api_key = activeKey;
    cfg[REG_MAIL_KEY_SLOTS[mailProvider]] = activeKey;
  }
  if ($("reg-api-key-clear") && $("reg-api-key-clear").checked && mailProvider === regMailClearProvider) {
    cfg.api_key_clear = true;
  }
  const yesCaptchaKey = !isLocal && $("reg-yescaptcha-key")
    ? $("reg-yescaptcha-key").value.trim()
    : "";
  if (yesCaptchaKey && !($("reg-yescaptcha-key-clear") && $("reg-yescaptcha-key-clear").checked)) {
    cfg.yescaptcha_key = yesCaptchaKey;
  }
  if ($("reg-yescaptcha-key-clear") && $("reg-yescaptcha-key-clear").checked) {
    cfg.yescaptcha_key_clear = true;
  }
  const proxyPassword = $("reg-proxy-password") ? $("reg-proxy-password").value.trim() : "";
  if (proxyPassword && !($("reg-proxy-password-clear") && $("reg-proxy-password-clear").checked)) {
    cfg.proxy_password = proxyPassword;
  }
  if ($("reg-proxy-password-clear") && $("reg-proxy-password-clear").checked) {
    cfg.proxy_password_clear = true;
  }
  return cfg;
}
...
function applyRegConfig(cfg) {
  ...
  const mailProv = mail === "yyds" ? "yyds" : mail === "gptmail" ? "gptmail" : mail === "cfmail" ? "cfmail" : mail === "tempmail" ? "tempmail" : mail === "cloudmail" || mail === "skymail" || mail === "cmail" ? "cloudmail" : "moemail";
  if ($("reg-mail-provider")) {
    $("reg-mail-provider").value = mailProv;
  }
  // Secret fields are write-only. The API exposes only *_set booleans, which
  // drive placeholders/status without ever restoring secret material.
  for (const [prov, slot] of Object.entries(REG_MAIL_KEY_SLOTS)) {
    if (Object.prototype.hasOwnProperty.call(cfg, slot + "_set")) {
      regMailKeySet[prov] = !!cfg[slot + "_set"];
    } else if (prov === mailProv && Object.prototype.hasOwnProperty.call(cfg, "api_key_set")) {
      regMailKeySet[prov] = !!cfg.api_key_set;
    }
  }
  ...
  if ($("reg-yescaptcha-key")) {
    $("reg-yescaptcha-key").value = "";
    $("reg-yescaptcha-key").placeholder = cfg.yescaptcha_key_set
      ? "已保存，留空保留"
      : "仅 YesCaptcha 模式需要";
  }
  if ($("reg-proxy")) $("reg-proxy").value = cfg.proxy || "";
  if ($("reg-proxy-username")) $("reg-proxy-username").value = cfg.proxy_username || "";
  if ($("reg-proxy-password")) {
    $("reg-proxy-password").value = "";
    $("reg-proxy-password").placeholder = cfg.proxy_password_set
      ? "已保存，留空保留"
      : "可选，共享认证";
  }
...
function resetRegSecretControls() {
  for (const ids of Object.values(REG_MAIL_INPUT_IDS)) {
    if (ids.key && $(ids.key)) $(ids.key).value = "";
  }
  for (const id of ["reg-yescaptcha-key", "reg-proxy-password"]) {
    if ($(id)) $(id).value = "";
  }
  for (const id of ["reg-api-key-clear", "reg-yescaptcha-key-clear", "reg-proxy-password-clear"]) {
    if ($(id)) $(id).checked = false;
  }
  regMailClearProvider = null;
}

function registrationConfigForLocalCache(cfg) {
  const source = cfg && typeof cfg === "object" ? cfg : {};
  const safeKeys = [
    "mail_provider", "provider", "base_url", "moemail_base_url", "cfmail_base_url", "cloudmail_base_url",
    "domain", "moemail_domain", "yyds_domain", "gptmail_domain", "cfmail_domain", "tempmail_domain", "cloudmail_domain",
    "expiry_ms", "captcha_provider", "local_solver_url", "proxy_strategy",
    "count", "concurrency", "stagger_ms", "probe_delay_sec",
    "api_key_set", "moemail_api_key_set", "yyds_api_key_set", "gptmail_api_key_set",
    "cfmail_api_key_set", "tempmail_api_key_set", "cloudmail_api_key_set", "yescaptcha_key_set", "proxy_password_set",
  ];
  const projected = {};
...
function cacheRegConfigLocal(cfg) {
  try {
    localStorage.setItem(REG_CONFIG_KEY, JSON.stringify(registrationConfigForLocalCache(cfg || readRegConfig())));
  } catch (_) {}
}
...
async function saveRegConfig() {
  ...
  try {
    ...
    const saved = (r && r.config) || {};
    const mail = String(saved.mail_provider || cfg.mail_provider || "moemail").toLowerCase();
    // Preserve submitted non-secret values when a response omits them. Secret
    // values are deliberately never merged into the response/cache/UI state.
    for (const k of [
      "mail_provider", "base_url", "moemail_base_url", "cfmail_base_url", "cloudmail_base_url",
      "domain", "moemail_domain", "yyds_domain", "gptmail_domain", "cfmail_domain", "tempmail_domain", "cloudmail_domain",
      "expiry_ms", "captcha_provider", "local_solver_url", "proxy_strategy",
      "count", "concurrency", "stagger_ms", "probe_delay_sec",
    ]) {
      if (!Object.prototype.hasOwnProperty.call(saved, k) && Object.prototype.hasOwnProperty.call(cfg, k)) {
        saved[k] = cfg[k];
      }
    }
    const activeSlot = REG_MAIL_KEY_SLOTS[mail] || REG_MAIL_KEY_SLOTS.moemail;
    if (cfg.api_key_clear) regMailKeySet[mail] = false;
    else if (cfg[activeSlot]) regMailKeySet[mail] = true;
    if (!Object.prototype.hasOwnProperty.call(saved, activeSlot + "_set")) {
      saved[activeSlot + "_set"] = regMailKeySet[mail];
    }
    if (cfg.yescaptcha_key_clear) saved.yescaptcha_key_set = false;
    else if (cfg.yescaptcha_key) saved.yescaptcha_key_set = true;
    if (cfg.proxy_password_clear) saved.proxy_password_set = false;
    else if (cfg.proxy_password) saved.proxy_password_set = true;
    resetRegSecretControls();
    ...
    // Keep typed secrets only in their write-only inputs for an explicit retry.
    // Never persist them to localStorage or application state.
    toast((e && e.message) || "保存失败（敏感值未写入本地缓存）", false);
    throw e;
  } finally {
...
function buildRegBody(config) {
      ...
      : mailProvider === "gptmail"
        ...
        : mailProvider === "cfmail"
          ...
          : mailProvider === "tempmail" || mailProvider === "tempmail.lol" || mailProvider === "lol"
            ? "tempmail"
            : mailProvider === "cloudmail" || mailProvider === "skymail" || mailProvider === "cmail"
              ? "cloudmail"
              : "moemail";
  // Keep legacy field for older backends.
  body.provider = body.mail_provider;
  // MoeMail + CF Temp Email + CloudMail need base_url; YYDS/GPTMail/TempMail.lol use fixed hosts.
  ...
  // Active key/domain are sent only when explicitly entered. Blank secrets are
  // omitted so the server retains them; explicit *_clear flags are forwarded.
  if (config.api_key) body.api_key = String(config.api_key);
  if (body.mail_provider === "moemail") {
    if (config.moemail_api_key) body.moemail_api_key = String(config.moemail_api_key);
    ...
    if (config.yyds_api_key) body.yyds_api_key = String(config.yyds_api_key);
    ...
    if (config.gptmail_api_key) body.gptmail_api_key = String(config.gptmail_api_key);
    body.gptmail_domain = config.gptmail_domain == null ? body.domain : String(config.gptmail_domain);
    body.domain = body.gptmail_domain;
    body.moemail_base_url = "";
    body.base_url = "";
  } else if (body.mail_provider === "cfmail") {
    if (config.cfmail_api_key) body.cfmail_api_key = String(config.cfmail_api_key);
    ...
    if (config.tempmail_api_key) body.tempmail_api_key = String(config.tempmail_api_key);
    body.tempmail_domain = config.tempmail_domain == null ? body.domain : String(config.tempmail_domain || "");
    body.domain = body.tempmail_domain;
    body.moemail_base_url = "";
    body.base_url = "";
  } else if (body.mail_provider === "cloudmail") {
    if (config.cloudmail_api_key) body.cloudmail_api_key = String(config.cloudmail_api_key);
    body.cloudmail_domain = config.cloudmail_domain == null ? body.domain : String(config.cloudmail_domain || "");
    body.domain = body.cloudmail_domain;
    const cmBase = config.cloudmail_base_url != null ? String(config.cloudmail_base_url) : (config.base_url != null ? String(config.base_url) : "");
    body.cloudmail_base_url = cmBase;
    body.base_url = cmBase;
    // Do not bleed CloudMail host into MoeMail slot.
    body.moemail_base_url = "";
  }
  if (config.api_key_clear) body.api_key_clear = true;
  ...
  if (config.yescaptcha_key_clear) body.yescaptcha_key_clear = true;
  ...
  if (config.proxy_password_clear) body.proxy_password_clear = true;
...
async function pollRegSession() {
  ...
  try {
    ...
    regPollTimer = null;
    try {
      // Force protected dashboard pool totals refresh after registration imports land in DB.
      _statusFetchedAt = 0;
      dashCache = await api("/dashboard");
      _statusFetchedAt = Date.now();
      if (window.G2A && G2A.state) G2A.state.dashboard = dashCache;
...
function bindSoftNav() {
  document.addEventListener("click", (e) => {
    ...
    e.preventDefault();
    softNavigate(page, { href });
  }, true);

  window.addEventListener("popstate", () => {
    const page = pageFromPath(location.pathname);
    if (page === "login") return;
    softNavigate(page, { replace: true, force: true, href: location.pathname + location.search });
  });
}
bindSoftNav();
...
function fillSystemSettingsForm(s) {
  ...
  if ($("set-outbound-proxy-password")) {
    ...
    const hasPw = !!(ob.proxy_password_set || ob.has_password);
    $("set-outbound-proxy-password").placeholder = hasPw ? "已保存，留空不改" : "可选，共享认证";
  }
  if ($("set-outbound-proxy-password-clear")) {
    $("set-outbound-proxy-password-clear").checked = false;
  }
  if ($("set-outbound-proxy-strategy")) {
...
function collectSystemSettingsPatch(groups) {
  ...
  if (want("proxy")) {
    ...
    if ($("set-outbound-proxy-password")) {
      ...
      const clear = !!($("set-outbound-proxy-password-clear") && $("set-outbound-proxy-password-clear").checked);
      if (clear) patch.outbound_proxy_password_clear = true;
      else if (pw) patch.outbound_proxy_password = pw;
    }
...
function applySettingsGroupDefaults(group) {
  ...
  if (group === "pool") {
    ...
    if ($("set-outbound-proxy-password-clear")) {
      $("set-outbound-proxy-password-clear").checked = true;
    }
