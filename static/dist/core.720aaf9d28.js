...
const PAGE_META = {
  ...
  accounts: { title: "账号", sub: "Grok 账号、设备码登录、额度与导入导出" },
  rotation: { title: "轮询", sub: "账号池调度策略、可用性与后台维护状态" },
  ...
  settings: { title: "系统设置", sub: "安全、集成与后台维护参数" },
...
const PAGE_HREF = { overview: "/admin", keys: "/admin/keys", accounts: "/admin/accounts", rotation: "/admin/rotation", usage: "/admin/usage", logs: "/admin/logs", models: "/admin/models", settings: "/admin/settings", guide: "/admin/guide" };
let _softNavToken = 0;
...
function pageFromPath(pathname) {
  ...
  if (p.endsWith("/rotation")) return "rotation";
...
async function softNavigate(name, opts) {
  ...
  try {
    ...
    document.documentElement.style.background = theme === "light" ? "#f6f1e7" : "#171713";
  } catch (_) {
    document.documentElement.style.background = "#171713";
  }
  ...
  try {
    ...
    if (page === "overview" || page === "rotation") {
    ...
    try {
      ...
      if (page === "rotation" && typeof renderRotationPool === "function") renderRotationPool();
...
function buildMobileNav() {
  ...
  if (!host) return;
  const map = { overview: "/admin", keys: "/admin/keys", accounts: "/admin/accounts", rotation: "/admin/rotation", usage: "/admin/usage", logs: "/admin/logs", models: "/admin/models", settings: "/admin/settings", guide: "/admin/guide" };
...
async function bootstrap() {
  ...
  try {
    ...
    try {
      ...
      if (page === "rotation") { try { renderRotationPool(); } catch (_) {} }
      if (page === "overview") { try { renderStats(); } catch (_) {} }
    }
    try { rebindPageControls(); } catch(_){}
    if (page === "overview" || page === "rotation") startAutoUiRefresh();
    if (page === "accounts") {
      renderAccounts();
      try {
...
async function refreshOverviewStatus({ force = true, render = true } = {}) {
  // Force-refresh protected dashboard data after admin actions.
  try {
...
async function loadDashboard() {
  ...
  if (page === "overview" || page === "rotation") {
    const renderPoolPage = () => {
      if (page === "rotation") renderRotationPool();
      else renderStats();
    };
    // Paint from /status immediately. /dashboard is optional enrichment only.
    try { renderPoolPage(); } catch (e) { console.error(e); }
    ...
    try {
      ...
      if (e && e.status === 401 && !e.soft) throw e;
      console.warn("guide dashboard failed", e);
    }
...
function renderStats() {
  ...
  $("stats-grid").innerHTML = `
    <div class="stat stat--endpoint"><div class="label">API Endpoint</div><div class="value mono">${esc(d.api_base || s.api_base || "")}</div></div>
    <div class="stat stat--version"><div class="label">CLI 版本</div><div class="value mono">${esc(d.cli_version || s.cli_version || "")}</div>
      <div class="sub">上游 ${esc(d.upstream || s.upstream || "")}</div></div>
    <div class="stat stat--pool"><div class="label">账号池</div><div class="value">${pool.live ?? pool.enabled ?? acc.active_count ?? 0} 可轮询 <span class="g2a-muted">/ ${pool.total ?? acc.account_count ?? 0} 总量</span></div>
      <div class="sub">${esc(d.account_mode || s.account_mode || "—")} · 冷却 ${pool.in_cooldown ?? 0} · 过期 ${pool.expired ?? 0} · 模型封禁 ${pool.model_blocked ?? 0} · 额度禁用 ${pool.quota_disabled ?? 0} · 停用 ${pool.disabled ?? 0}</div></div>
    <div class="stat"><div class="label">API Keys</div><div class="value">${keys.enabled ?? 0} 启用 <span class="g2a-muted">/ ${keys.total ?? 0}</span></div>
      <div class="sub">累计请求 ${keys.total_requests ?? 0} · 鉴权 ${keys.auth_required ? "开启" : "关闭"}</div></div>
    <div class="stat"><div class="label">今日用量</div><div class="value mono">${fmtNum((d.usage || s.usage || {}).today_tokens || 0)} token</div>
      <div class="sub">请求 ${(d.usage || s.usage || {}).today_requests ?? 0} · 累计 ${fmtNum((d.usage || s.usage || {}).total_tokens ?? 0)} token</div></div>
    <div class="stat ${(tm.running || tm.cluster_running || tm.leader_running) ? "stat--healthy" : "stat--warning"}"><div class="label">Token 自动续期</div><div class="value">${(tm.running || tm.cluster_running || tm.leader_running) ? "运行中" : (tm.enabled === false ? "已关闭" : (tm.enabled ? "已启用" : "未运行"))}</div>
...
function renderRotationPool() {
  const host = $("rotation-stats");
  if (!host) return;
  const s = statusCache || {};
  const d = dashCache || {};
  const pool = d.pool || s.pool || {};
  const acc = d.accounts || s.accounts || {};
  const mode = d.account_mode || s.account_mode || "round_robin";
  const total = pool.total ?? acc.account_count ?? 0;
  const live = pool.live ?? pool.enabled ?? acc.active_count ?? 0;
  const cooling = pool.in_cooldown ?? 0;
  const unavailable = (pool.expired ?? 0) + (pool.model_blocked ?? 0) + (pool.quota_disabled ?? 0) + (pool.disabled ?? 0);
  const select = $("account-mode");
  if (select && document.activeElement !== select) select.value = mode;
  const modePill = $("rotation-mode-pill");
  if (modePill) {
    modePill.className = "g2a-tag ok";
    modePill.textContent = mode;
  }
  host.innerHTML = `
    <div class="stat stat--pool"><div class="label">可轮询</div><div class="value">${live} <span class="g2a-muted">/ ${total}</span></div><div class="sub">当前可参与请求调度的账号</div></div>
    <div class="stat ${cooling ? "stat--warning" : "stat--healthy"}"><div class="label">冷却中</div><div class="value">${cooling}</div><div class="sub">单次测活成功后可重新入池</div></div>
    <div class="stat ${unavailable ? "stat--warning" : "stat--healthy"}"><div class="label">不可用</div><div class="value">${unavailable}</div><div class="sub">过期 ${pool.expired ?? 0} · 模型封禁 ${pool.model_blocked ?? 0} · 额度禁用 ${pool.quota_disabled ?? 0} · 停用 ${pool.disabled ?? 0}</div></div>
    <div class="stat"><div class="label">调度策略</div><div class="value mono">${esc(mode)}</div><div class="sub">更改策略后立即对新请求生效</div></div>
  `;
}
...
async function loadModels() {
  ...
  try {
    ...
    if (r && r.model_health) dashCache.model_health = r.model_health;
    if (r && r.settings) dashCache.settings = Object.assign({}, dashCache.settings || {}, r.settings);
  } catch (e) {
...
function startAutoUiRefresh() {
  ...
  uiRefreshTimer = setInterval(async () => {
    try {
      const page = document.body.dataset.page || pageFromPath(location.pathname) || "overview";
      if (page !== "overview" && page !== "rotation") return;
      ...
      try {
        const now = Date.now();
        if (!dashCache || (now - (_statusFetchedAt || 0)) > 5000) {
          dashCache = await api("/dashboard");
          _statusFetchedAt = Date.now();
          if (window.G2A && G2A.state) G2A.state.dashboard = dashCache;
        }
...
function buildRegBody(config) {
  ...
  if (body.captcha_provider === "local") {
    ...
    body.yescaptcha_key = config.yescaptcha_key == null ? "" : String(config.yescaptcha_key);
  }
...
function fillSystemSettingsForm(s) {
  ...
  if ($("set-upstream-retry-count")) {
    if (pol.upstream_retry_count != null) {
      $("set-upstream-retry-count").value = pol.upstream_retry_count;
    } else if (pol.max_failover_attempts != null) {
      $("set-upstream-retry-count").value = Math.max(0, Math.min(63, Number(pol.max_failover_attempts) - 1));
    }
  }
...
function collectSystemSettingsPatch() {
  ...
  if ($("set-upstream-retry-count") && $("set-upstream-retry-count").value !== "") patch.upstream_retry_count = Number($("set-upstream-retry-count").value);
  // Outbound proxy pool
  ...
  if ($("set-outbound-proxy-password")) {
    ...
    patch.outbound_proxy_password = pw;
  }
