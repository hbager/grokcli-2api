...
func New(store *postgres.Connector, upstream string) *Service {
	return &Service{
		...
		httpClient: newQuotaHTTPClient(nil),
	}
}

func newQuotaHTTPClient(proxyFn func(*http.Request) (*url.URL, error)) *http.Client {
...
// SetProxy atomically replaces the shared quota client so future probes use the
// current dynamic proxy selection callback.
func (s *Service) SetProxy(proxy func(*http.Request) (*url.URL, error)) {
	if s == nil {
		return
	}
	next := newQuotaHTTPClient(proxy)
	s.clientMu.Lock()
