...
func responsesToChatStream(body io.ReadCloser) io.ReadCloser {
	upstream := &onceReadCloser{ReadCloser: body}
	pr, pw := io.Pipe()
	go func() {
		err := translateOrPassthroughSSE(upstream, pw)
		_ = upstream.Close()
		if err != nil {
	...
	return &bridgeReadCloser{PipeReader: pr, upstream: upstream}
}
...
func (b *bridgeReadCloser) Close() error {
	b.once.Do(func() {
		if b.upstream != nil {
			b.err = b.upstream.Close()
		}
		if err := b.PipeReader.Close(); b.err == nil {
			b.err = err
		}
	})
	return b.err
