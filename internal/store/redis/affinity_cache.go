...
const localCacheLimit = 8192
const localCacheTrimTarget = 6144
...
func affinityCacheSet(fingerprint, accountID string) {
	...
	trimLocalCache(affinityCache, func(entry affinityCacheEntry) time.Time { return entry.expires })
...
func responseCacheSet(responseID, accountID, promptCacheKey string) {
	...
	trimLocalCache(responseCache, func(entry responseCacheEntry) time.Time { return entry.expires })
	affinityCacheMu.Unlock()
}

func trimLocalCache[T any](cache map[string]T, expires func(T) time.Time) {
	if len(cache) <= localCacheLimit {
		return
	}
	now := time.Now()
	for key, value := range cache {
		if now.After(expires(value)) {
			delete(cache, key)
		}
	}
	if len(cache) <= localCacheLimit {
	...
	entries := make([]expiry, 0, len(cache))
	for key, value := range cache {
		entries = append(entries, expiry{key: key, time: expires(value)})
	}
	sort.Slice(entries, func(i, j int) bool { return entries[i].time.Before(entries[j].time) })
	remove := len(cache) - localCacheTrimTarget
	for index := 0; index < remove; index++ {
		delete(cache, entries[index].key)
	}
}
